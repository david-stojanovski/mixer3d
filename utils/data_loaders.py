...
class HeartSegDataset(torch.utils.data.dataset.Dataset):
    ...
    def get_datum(self, idx):
        ...
        def fuse_labels(in_volume):
            # Fuse labels
            in_volume[in_volume == 240] = 40
            in_volume[in_volume == 230] = 40
            in_volume[in_volume == 170] = 40
            in_volume[in_volume == 160] = 40

            in_volume[in_volume == 220] = 30
            in_volume[in_volume == 210] = 30
            in_volume[in_volume == 200] = 30
            in_volume[in_volume == 190] = 30
            in_volume[in_volume == 180] = 30
            in_volume[in_volume == 150] = 30
            in_volume[in_volume == 140] = 30
            in_volume[in_volume == 130] = 30
            in_volume[in_volume == 120] = 30
            in_volume[in_volume == 110] = 30

            in_volume[in_volume == 60] = 20

            # ## fusing valves
            # in_volume[in_volume == 90] = 10
            # in_volume[in_volume == 100] = 20
            # in_volume[in_volume == 70] = 30
            # in_volume[in_volume == 80] = 40
        ...
        if suffix == '.mat':
            ...
            volume = fuse_labels(volume)
            volume[volume > 40] = volume[volume > 40] - 20
        ...
        return taxonomy_name, sample_name, np.asarray(rendering_images), volume.astype(np.float32) / 10
...
# //////////////////////////////// = End of HeartSegDataset Class Definition = ///////////////////////////////// #
...
class UltrasoundDataset(torch.utils.data.dataset.Dataset):
...
class UltraSoundDataLoader:
    def __init__(self, cfg):
        self.dataset_taxonomy = None
        self.rendering_image_path_template = cfg.DATASETS.ULTRASOUND.RENDERING_PATH

        file_paths_in_dir = natsorted(glob.glob(self.rendering_image_path_template % ('*', '*')))
        file_names = [file_name.split(os.sep)[-2] for file_name in file_paths_in_dir]
        self.dataset_taxonomy = [{'taxonomy_id': 'inference', 'taxonomy_name': 'inference',
                                  'inference_cases': set(file_names)}]
    ...
    def get_dataset(self, n_views_rendering, transforms=None):
        ...
        for taxonomy in self.dataset_taxonomy:
            ...
            samples = taxonomy['inference_cases']

            files.extend(self.get_files_of_taxonomy(taxonomy_folder_name, samples))

        print('[INFO] %s Complete collecting files of the dataset. Total files: %d.' % (dt.now(), len(files)))
        return UltrasoundDataset(files, n_views_rendering, transforms)
...
DATASET_LOADER_MAPPING = {
    ...
    'UltraSound': UltraSoundDataLoader
}  # yapf: disable
